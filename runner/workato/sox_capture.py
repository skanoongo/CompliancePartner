...
-----
  ...
  python3 -m workato.sox_capture

  # options
  python3 -m workato.sox_capture --month "September 2026"
  python3 -m workato.sox_capture --capture-only        # screenshots + manifest only
  python3 -m workato.sox_capture --build-only          # rebuild workbook from last manifest
  python3 -m workato.sox_capture --browser chrome      # use installed Google Chrome instead of bundled Chromium
  python3 -m workato.sox_capture --no-pause            # skip the "confirm workspace" prompt
...
from core.platform import (  # noqa: F401 - re-exported for this package
    APP_NAME,
    IS_LINUX,
    IS_MAC,
    IS_WINDOWS,
